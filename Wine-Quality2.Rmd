...
```{r echo=FALSE}
# Read in the data
folder = "C:/Users/Laura Olson/Downloads/Math"
...
The distribution of quality of wine is slightly skewed to the left, according to the box plot. Because in the histogram the data seem fairly symetric, we look for an alternate explanation for the appearance of the box plot. The width of the middle two quartiles is only a single unit, therefore, we know that the only option for the median is for it to be at the same value as either the first or third quartile. As a result, we do not consider transforming the response variable, quality.
...
```{r}
fit.intRed2 = lm (quality ~ volatile_acidity.c + log_chlorides + free_sulfur_dioxide + total_sulfur_dioxide.c + pH + sulphates.c + alcohol.c + volatile_acidity.c*total_sulfur_dioxide.c + total_sulfur_dioxide.c*sulphates.c + sulphates.c*alcohol.c )
summary(fit.intRed2)
anova(fit.intRed2)
```

All interaction effects are significant when added to our reduced first-order model. Volatile_acidity x total_sulphur dioxide remained significant to a false positive rate of $\alpha = .001$ (p-value = 0.0008). Total_sulfur_dioxide x sulphates on the other hand gained significance in the reduced full-order model where it is now signicant at $\alpha = .001$ (p-value = 3.66e-08) compared to the previous model where it was significant only to $\alpha = .05$. Sulphates x alcohol also gained significance to the rate of $\alpha = .001$ (p-value = 0.000574).
...
We examine the interaction effect of the most significant cross-product predictor total sulfur dioxide with sulphates from our reduced first-order model.
...
For the second most significant of three interaction effects:

```{r}
# Categorize sulphates
# Obtain 1st quartile, median, and 3rd quartile
ss = summary(total_sulfur_dioxide.c)
ss
```

```{r}
wine$total_sulph.cat = "Low"
wine$total_sulph.cat[total_sulfur_dioxide.c > ss[2]] = "Medium-Low"
wine$total_sulph.cat[total_sulfur_dioxide.c > ss[3]] = "Medium"
wine$total_sulph.cat[total_sulfur_dioxide.c > ss[4]] = "High"
plot(jitter(quality,1) ~ volatile_acidity.c, col=as.factor(wine$total_sulph.cat), xlab="Centered Volatile Acidity", ylab="Quality")
legend(0.6, 7.5, c("Low", "Medium-Low", "Medium", "High"), pch=rep(1,4), col=1:4)
# Low
abline(lm(quality[wine$total_sulph.cat == "Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Low"]), col=1)
# Medium-Low
abline(lm(quality[wine$total_sulph.cat == "Medium-Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium-Low"]), col=2)
# Medium
abline(lm(quality[wine$total_sulph.cat == "Medium"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium"]), col=3)
# High
abline(lm(quality[wine$total_sulph.cat == "High"] ~ volatile_acidity.c[wine$total_sulph.cat == "High"]), col=4)
```

The interaction plot of Quality vs. Centered Volatile Acidity by Total Sulfur Dioxide level shows that wine quality decreases fastest with increasing total sulfur dioxide for the Low sulfur dioxide group. Additionally, wine quality decreases with increasing centered volatile acidity for the Medium-Low and Medium levels, but at slower rates than the Low group. Wine quality decreases the least quickly with volatile acidity for the High sulfur dioxide group.
...
par(mfrow=c(2,2))
# Low
plot(quality[wine$total_sulph.cat == "Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Low"], col=c("black"), xlab="Volatile acidity when total sulfur dioxide is Low", ylab="Quality")
legend(0.6, 7.5, c("Low"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$total_sulph.cat == "Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Low"]), col=1)
# Medium-Low
plot(quality[wine$total_sulph.cat == "Medium-Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium-Low"], col=c("red"), xlab="Volatile acidity when total sulfur dioxide is Medium-Low", ylab="Quality")
legend(0.15, 7.5, c("Medium-Low"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$total_sulph.cat == "Medium-Low"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium-Low"]), col=1)
# Medium
plot(quality[wine$total_sulph.cat == "Medium"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium"], col=c("green"), xlab="Volatile acidity when total sulfur dioxide is Medium", ylab="Quality")
legend(0.3, 7.5, c("Medium"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$total_sulph.cat == "Medium"] ~ volatile_acidity.c[wine$total_sulph.cat == "Medium"]), col=1)
# High
plot(quality[wine$total_sulph.cat == "High"] ~ volatile_acidity.c[wine$total_sulph.cat == "High"], col=c("blue"), xlab="Volatile acidity when total sulfur dioxide is High", ylab="Quality")
legend(0.4, 7.5, c("High"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$total_sulph.cat == "High"] ~ volatile_acidity.c[wine$total_sulph.cat == "High"]), col=1)
```

Although this plot relays the same information about the slopes of wine quality versus total sulphur dioxide by each sulphur level as the previous interaction plot displayed, it is easier to see each individual interaction effect as seperated by sulfur dioxide level.

For the third most significant of three interaction effects:

```{r}
# Categorize sulphates
# Obtain 1st quartile, median, and 3rd quartile
ss = summary(alcohol.c)
ss
```

```{r}
wine$alcohol.cat = "Low"
wine$alcohol.cat[alcohol.c > ss[2]] = "Medium-Low"
wine$alcohol.cat[alcohol.c > ss[3]] = "Medium"
wine$alcohol.cat[alcohol.c > ss[4]] = "High"
plot(jitter(quality,1) ~ sulphates.c, col=as.factor(wine$alcohol.cat), xlab="Centered Sulphates", ylab="Quality")
legend(0.9, 5, c("Low", "Medium-Low", "Medium", "High"), pch=rep(1,4), col=1:4)
# Low
abline(lm(quality[wine$alcohol.cat == "Low"] ~ sulphates.c[wine$alcohol.cat == "Low"]), col=1)
# Medium-Low
abline(lm(quality[wine$alcohol.cat == "Medium-Low"] ~ sulphates.c[wine$alcohol.cat == "Medium-Low"]), col=2)
# Medium
abline(lm(quality[wine$alcohol.cat == "Medium"] ~ sulphates.c[wine$alcohol.cat == "Medium"]), col=3)
# High
abline(lm(quality[wine$alcohol.cat == "High"] ~ sulphates.c[wine$alcohol.cat == "High"]), col=4)
```

The interaction plot of Quality vs. Centered Centered Sulphates by Alcohol level shows that wine quality increases fastest with increasing centered sulphates for the High alcohol group. Additionally, wine quality increases less quickly with increasing centered sulphates for the Medium, Medium-Low, and Low levels, respectively.

```{r}
par(mfrow=c(2,2))
# Low
plot(quality[wine$alcohol.cat == "Low"] ~ sulphates.c[wine$alcohol.cat == "Low"], col=c("black"), xlab="Sulphates when alcohol is Low", ylab="Quality")
legend(1, 4.5, c("Low"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$alcohol.cat == "Low"] ~ sulphates.c[wine$alcohol.cat == "Low"]), col=1)
# Medium-Low
plot(quality[wine$alcohol.cat == "Medium-Low"] ~ sulphates.c[wine$alcohol.cat == "Medium-Low"], col=c("red"), xlab="Sulphates when alcohol is Medium-Low", ylab="Quality")
legend(0.45, 5.0, c("Medium-Low"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$alcohol.cat == "Medium-Low"] ~ sulphates.c[wine$alcohol.cat == "Medium-Low"]), col=1)
# Medium
plot(quality[wine$alcohol.cat == "Medium"] ~ sulphates.c[wine$alcohol.cat == "Medium"], col=c("green"), xlab="Sulphates when alcohol is is Medium", ylab="Quality")
legend(0.2, 5, c("Medium"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$alcohol.cat == "Medium"] ~ sulphates.c[wine$alcohol.cat == "Medium"]), col=1)
# High
plot(quality[wine$alcohol.cat == "High"] ~ sulphates.c[wine$alcohol.cat == "High"], col=c("blue"), xlab="Sulphates when alcohol is High", ylab="Quality")
legend(0.3, 4.7, c("High"), pch=rep(1,4), col=1:4)
abline(lm(quality[wine$alcohol.cat == "High"] ~ sulphates.c[wine$alcohol.cat == "High"]), col=1)
```
...
```{r}
fit.sbc.final = lm(quality ~ volatile_acidity.c + citric_acid + log_chlorides + total_sulfur_dioxide.c + pH + sulphates.c + alcohol.c + volatile_acidity.c:total_sulfur_dioxide.c + total_sulfur_dioxide.c:sulphates.c + sulphates.c:alcohol.c) 
summary(fit.sbc.final)
```
 
All predictors were found to be significant to a false positive rate of of $\alpha = .001$ except for citic acid is significant at $\alpha = .01$.

Quality decreases with each additional unit of volatile_acidity.c, citric acid, log(chlorides), total_sulfur_dioxide.c, pH, and total sulphur dioxide x sulphates. The steepest negative slope is volatile acidity at -1.05 while the other negative slopes range between -0.002 and -0.20. Conversely, quality increases with each additional unit of sulphates, alcohol, volatile acidity x total sulphur dioxide, and sulphates x alcohol. The steepest positive slope is sulphates at 1.19 while the other positive slopes lie between 0.009 and 0.37.
...
```{r}
cbind.data.frame (Model=c("All predictors (w/ transformations)", "Manual Backward Elimination", "Manual Backward Elmination with added interaction effects", "Stepwise with SBC criterion"),
                  Adj.R2 = c(summary(fit2)$adj.r.squared,
                            summary (fit.myReduced)$adj.r.squared,
                             summary (fit.intRed2)$adj.r.squared,
                             summary (fit.sbc.final)$adj.r.squared),
                  Sigma.hat = c(summary(fit2)$sigma,
                                summary (fit.myReduced)$sigma,
                                summary(fit.intRed2)$sigma,
                                summary(fit.sbc.final)$sigma))
```

Though all the models output low adjusted $R^2$ values and relatively low residual standard error values, the stepwise model with the SBC criterion does have the highest adjusted $R^2$ and lowest residual standard error values. We note that the stepwise model with SBC criterion's adjusted $R^2$ and residual standard error values are very similar to the adjusted $R^2$ and residual standard error values of our manual backward elimination model with added interaction effects. Upon further comparsion of these two similar models, we note that the predictors for each model are the same except for the absence of the variable free sulphur dioxide in the stepwise model with SBC and the the absence of citric acid in our manual backward elimination model with added interaction effects. We also find that although these two models share much resemblance, every predictor that is shared between the two has a greater p-value in the stepwise regression model than in the manual backward elimination model.

Due to the slightly higher $R^2$ and p-values as well as slightly lower residual standard error. We will designate this model as the best model for predicting the quality of red Portuguese "Vinho Verde" wine.
...
```{r}
plot (fit.sbc.final$fitted.values, rstudent (fit.sbc.final),
...
```{r}
par (mfrow = c(1,2))
boxplot (fit.sbc.final$residuals, main="Final Model Residuals")
plot (fit.sbc.final, which=2, main="Normal Q-Q")
```
...
```{r}
bptest(fit.sbc.final, varformula = NULL, studentize = TRUE, data = list())
```
...
pairs (data.frame(quality, volatile_acidity.c, citric_acid, log_chlorides, total_sulfur_dioxide.c, pH, sulphates.c, alcohol.c, volatile_acidity.c:total_sulfur_dioxide.c, total_sulfur_dioxide.c:sulphates.c, sulphates.c:alcohol.c), col=hatcolor)
#length(which(hatcolor == 2))
```

The hat matrix diagonals method identified over 100 high leverage points, indicating the presence of many outliers in the data set. However, a great majory of the identified potential outliers are not outlying at all, but rather reside in the middle of all the observations. We will deter removing all 135 outliers until we have investigated other outlier and influencial identifying methods. 

### DFFITS

```{r}
# Cutoff for leverage values
n.fit.final = fit.sbc.final$df.residual + fit.sbc.final$rank
Leverage.cut = 2 * sqrt(fit.sbc.final$rank / (n.fit.final))
Leverage.cut
```

The high leverage cutoff value is $2\sqrt{p/n}=0.166$. We will apply this leverage to our final model and plot the results.

```{r}
par (mfrow = c(1,1))
wine$dffits = dffits (fit.sbc.final)
wine$quality.fit = 10^fit.sbc.final$fitted.values
...
```{r}
cookd = cooks.distance (fit.sbc.final)
...
```{r}
vif(fit.sbc.final)
mean (vif(fit.sbc.final))
```

We note that the variance inflation factors range from 1.05 to 1.63 and that all variance inflation factors are less than 10--a good sign that there are no indications that multicollinearity may be excessively influencing the least squares estimates of our model. Citric acid and volatile acidity have the highest inflation values, 2.172 and 1.633 respectively, which we already know to be highly correlated. The mean of the VIFs is 1.434. Although this is larger than one, we do not deem 1.434 to be "considerably larger than one", and conclude that there are no serious multicollinearity issues in our final model. 
...
fit.sbc.final.ex = lm (quality ~ volatile_acidity.c + citric_acid + log_chlorides + total_sulfur_dioxide.c + pH + sulphates.c + alcohol.c + volatile_acidity.c:total_sulfur_dioxide.c + total_sulfur_dioxide.c:sulphates.c + sulphates.c:alcohol.c, data=wine.ex)
plot(fit.sbc.final$fitted.values[-c(87, 92, 152, 653, 1080, 1082)], fit.sbc.final.ex$fitted.values, xlab="Final Model with Potential Influential Observations Removed", ylab="Final Model", main="Wine Quality of Final Model vs. Final Model with Removed Observations")
```
...
```{r}
summary(fit.sbc.final.ex)
```

Comparing the summary output of the final model and the final model with the potentially influential observations removed, all predictors significant to a false positive rate of $\alpha = .001$ in our final model (fit.sbc.final) are still signifcant at $\alpha = .001$ in the remedial model with the outlying observations removed except for log(chlorides) which dropped down to $\alpha = .01$ significance level.

Quality still decreases with each additional unit of volatile_acidity.c, citric acid, log(chlorides), total_sulfur_dioxide.c, pH, and total sulphur dioxide x sulphates and increases with each additional unit of sulphates, alcohol, volatile acidity x total sulphur dioxide, and sulphates x alcohol. The steepest negative slope is still volatile acidity at -1.03, however, this is slightly less steep than -1.05 from the original final model. The steepest positive slope is still sulphates, with a slightly faster rate of 1.21 compared to the final model's 1.19.

The adjusted $R^2$ for this model is 0.393. This is a slight improvement of about .01 from the final model without the notable observations removed. The residual standard error is 0.628, a decrease of .01 from the final model. 

Given that the plot of the final model versus the remedial model with observations removed was very linear and that no significant improvements in adjusted $R^2$ and the residual standard error were made, we will not remove the observations indicated as outliers by the DFFits method.

## Remedial Measures

Given that the plot of the final model versus the remedial model with potentially influencial observations removed was very linear and that no significant improvements in adjusted $R^2$ and the residual standard error were made, we will proceed with our original final model (potentially influential observations intact)--no remedial measures needed.
...
```{r}
pred.all = as.data.frame (predict (fit.sbc.final, interval="prediction"))
pred.all2 = round (pred.all, 1)
pred.table = cbind.data.frame (wine$quality, wine$volatile_acidity.c, wine$citric_acid, wine$log_chlorides, wine$total_sulfur_dioxide.c, wine$pH, wine$sulphates.c, wine$alcohol.c, wine$volatile_acidity.c*total_sulfur_dioxide.c, wine$total_sulfur_dioxide.c*sulphates.c, wine$sulphates.c*alcohol.c, pred.all2$fit, pred.all2$lwr, pred.all2$upr)
names (pred.table) [12:14] = c("Fit", "Lower", "Upper")
names(pred.table) [1] = c("quality")
pred.table$MOE = (pred.table$Upper - pred.table$Lower) / 2
pred.table [c(900, 1300, 410, 575, 188, 462, 1587, 119, 1280, 888, 97, 1550),]
```

The biggest takeaway from these predictions is that no matter the quality of wine, the model can predict the quality level for any given red wine variant with a precision of about 1.25 quality units.

We note that every prediction fell within the confidence interval and that there doesn't appear to be a trend in the accuracy of predicting the qualtiy of red wine. That is, the accuracy of predicting red wine variants doesn't increase nor increase depending on the level of quality being predicted. 

A quick sanity check to make sure that with repeated sampling, 95% of the time, our prediction for a red wine variant's quality level will fall within the 95% confidence limits of our regression line.
...
Wine quality decreases with each additional unit of volatile acidity, citric acid, log(chlorides), total sulfur dioxide, density, pH, and total sulphur dioxide with sulphates.

Conversely, quality increases with each additional unit of fixed acidity, log(residual sugar), free sulphur dioxide, sulphates, alcohol, volatile acidity with total sulphur dioxide, and sulphates with alcohol. 
 
The both directions stepwise regression model with the SBC criterion ended with the model:

Quality varies with centered volatile acidity, citric acid, log of chlorides, centered total sulfur dioxide, pH, centered sulphates, centered alcohol, centered volatile acidity with centered total sulfur dioxide, centered total sulfur dioxide with centered sulphates, and centered sulphates with centered alcohol.

All primary predictors in the model that are involved in an interaction effect are present in the model. Notably, the stepwise regression model decided to keep citric acid as a predictor which we had removed while doing backwards elimination.

Though all of our models had relatively low residual standard error values, they all had relatively low adjusted $R^2$ values and the stepwise model with the SBC criterion does have the hightest adjusted $R^2$ value by a small margin $R^2$ and the lowest residual standard eror. Due to these slight benefits the stepwise model has over the the other models we have gathered, we selected the stepwise model as our final model.

...
The hat matrix diagonals method identified 135 high leverage points, indicating the presence of many outliers in the data set. However, a great majory of the identified potential outliers are not outlying at all, but rather reside in the middle of all the observations. COMPARE WITH STUDENTIZED RESIDUAL PLOT.
We will deter removing all 135 outliers until we have investigated other outlier and influencial identifying methods. 
...
wine [abs(wine$dffits) > 0.5, c(15,2,13,18,9,20,21)] 

```

The DFFits plot shows some red wine variants with high DFFits values. From the output, we glean that there are 7 total rows with high DFFits values greater than 0.1937. We will now highlight these potential influenctial data points in red in the pairs plot below to visualize how they might be affecting our model.

```{r fig.height=8, fig.width=8}
dffitscolor = ifelse (abs(wine$dffits) > 0.5, 2,1)
...
By highlighting these potential influencial obeservations in a pairs plot, we can easily see that these seven data points noticeably stand out from the rest of the data, quite a smaller subset than the 135 identified by the Hat Matrix Diagonals method. We will take a look at one more method before attempting to removing outlying observations. 

### Cook's Distance 

```{r}
cookd = cooks.distance (fit.full_bic)
plot(cookd, xlab="Row Number", ylab="Cook's Distance", main="Leverage Values")
abline(h=0.06,col='red')

```

```{r}
cookscolors = ifelse (cookd > 0.06, 2,1)
...
Considerably more strict than DFFits, Cook's Distance method only identified two influencial observations, rows 653 and 1082. 
...
We will now remove the seven rows identified by DFFits and the two rows identified by Cook's Distance as influential observations and see the extend the which the removal of these observations have on our final model.

```{r}
wine2 = wine[c(-15,-2,-13,-18,-9,-20,-21, -653, -1082),]
fit.full_bic_remdial = lm (quality ~ fixed_acidity + volatile_acidity.c + citric_acid + log_residual_sugar + log_chlorides + free_sulfur_dioxide + total_sulfur_dioxide.c + density + pH + sulphates.c + alcohol.c + volatile_acidity.c*total_sulfur_dioxide.c + total_sulfur_dioxide.c*sulphates.c + sulphates.c*alcohol.c, data=wine2)

summary(fit.full_bic_remdial)
```

All predictors significant to a false positive rate of $\alpha = .001$ in our final model (fit.full_bic) are still signifcant at $\alpha = .001$ in the remedial model with the outlying observations removed excpet for log(chlorides) which dropped down to $\alpha = .01$ significance level. Noteably, pH is no longer a significant predictor and citric acid went from being significant at $\alpha = .05$ in the final model to being significat at $\alpha = .01$ in the remedial model. Free sulfur dixoide remained at the same significance level.

Quality still decreases with each additional unit of volatile_acidity.c, citric acid, log(chlorides), total_sulfur_dioxide.c, density, pH, and total sulphur dioxide x sulphates and increases with each additional unit of fixed acidity, log(residual sugar), free sulphur dioxide, sulphates, alcohol, volatile acidity x total sulphur dioxide, and sulphates x alcohol. The steepest negative slope is still pH, but a significantly lesser degree of -5.263e-01 compared to the final model of -9.841. The same issue lies with the steepest positive slope which is still sulphates, but at a rate of 1.21 compared to the final model's 1.195.

The adjusted $R^2$ for this model is 0.3925. This is a slight improvement of .01 from the final model. The residual standard error is 0.6294, a decrease of .01 from the final model. 

Because the removal of these potential outliers did not significantly effect our final model, we will proceed with our analysis of this final model with these observations intact.

## Remedial Measures
Wait for Laura's code
...
todo: From this final model, we can infer that...

# Predictions

Finally, we will create prediction intervals for individual red wine variants. We selected two red wine variants for for each level of quality from 3-8. (We selected levels 3-8 because there are no observations in the data se  where the quality of wine was 0, 1, 2, 9, or 10). 
...
The biggest takeaway from these predictions is that no matter the quality of wine, the model can predict the quality level for any given red wine variant with a precision of 1.25. 

We note that the prediction interval for row 900 (3.6-6.1) does not quite contain the actual quality value of 3, nor do rows 410 and 575 (4.4-6.9) where the actual quality of wine is 4, or rows 97 (4.0-6.5) and 1550 (5.1-7.6) where the acutal quality of wine is 8. We expect this to happen 5% because we're using 95% confidence. 
...
The added variable plots, below, show no discernable trend. The fact that the trend lines appear horizontal is in acordance with our $\hat{\beta}$ estimates, but we cannot discern if the sign of the trends corresponds to the $\hat{\beta}$ estimates or not, neither can we discern whether or not the slopes of the trend lines are actually zero or are simply very close to zero.
...
```
By the Breusch-Pagan test for non-constant variance, because the p-value is small (for $\alpha = .001$) we reject the null hypothesis there is constant variance, so we understand that there is nonconstant variance.
